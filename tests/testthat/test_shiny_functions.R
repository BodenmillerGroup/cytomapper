test_that("cytomapperShiny: Standard input testing works", {
    ...
    expect_error(cytomapperShiny(object = pancreasSCE, img_id = c("test", 2)), 
                 regexp = "Invalid argument for 'img_id'.",
                 fixed = TRUE)
    expect_error(cytomapperShiny(object = pancreasSCE, img_id = 2), 
                 regexp = "Invalid argument for 'img_id'.",
                 fixed = TRUE)
    expect_error(cytomapperShiny(object = pancreasSCE, img_id = "test"), 
                 regexp = "'img_id' not in 'colData(object)'.",
                 fixed = TRUE)
    cur_obj <- pancreasSCE
    rownames(cur_obj) <- NULL
    expect_error(cytomapperShiny(object = cur_obj, img_id = "ImageNb"), 
                 regexp = "Please specify the rownames of the 'object'.",
                 fixed = TRUE)
    rownames(cur_obj) <- paste0("test", 1:5)
    expect_error(cytomapperShiny(object = cur_obj, image = pancreasImages, img_id = "ImageNb"), 
    ...
    expect_error(cytomapperShiny(object = pancreasSCE, image = cur_images, img_id = "ImageNb"), 
