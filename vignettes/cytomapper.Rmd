...
This vignette gives an introduction to displaying imaging mass cytometry data with the `cytomapper` package.
...
It requires three
  ...
  contains the cells' counts and metadata, 2. an CytoImageList object containing
  the pixel-level information per channel and 3. an CytoImageList object
...
```{r quickstart-load-CytoImageList}
data(pancreasImages)
pancreasImages
```
...
cytomapper combines objects of the `r Biocpkg("SingleCellExperiment")` class and the `CytoImageList` class `r Biocpkg("cytomapper")` to visualize cell- and pixel-level information.

In the main functions of the package, `image` refers to an `CytoImageList` object containing one or multiple multi-channel images where each channel represents the pixel-intensity of one marker (protein, RNA, etc.).
The enrty `mask` refers to an `CytoImageList` object containing one or multiple segmentation masks.
...
To access the `elementMetadat` slot of an `CytoImageList` object, please refer to the section [The CytoImageList object](#CytoImageList).
