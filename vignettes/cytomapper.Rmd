...
`pancreasImages` object and can be read in from the tiff-stacks (see next
section). Entires to the `CytoImageList` object and the rownames of the object
match: A02_imc, D01_imc, and F01_imc The `elementMetadata` slot (accesible via
the `mcols()` function) contains the image identifiers.
...
The final `all_masks` object corresponds to the `pancreasMasks` object provided 
by `cytomapper`.
...
To clip the image range, the user can provide a clipping range.
...
```{r subsetting-pixels}
cur_images <- getImages(pancreasImages, "F01_imc")
plotPixels(cur_images, colour_by = c("H3", "SMA", "CD44"))
```
